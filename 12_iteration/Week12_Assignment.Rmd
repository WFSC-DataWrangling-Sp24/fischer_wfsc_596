...
a.  Use this function and a for loop to calculate the estimated mass for each dinosaur, store the masses in a vector, and after all of the calculations are complete show the first few items in the vector using `head()`.
    ...
    ```{r}
    masses <- vector(mode = "numeric", length = length(species))
...
b.  Add the results in the vector back to the original data frame. Show the first few rows of the data frame using `head()`.
    ...
    dino_masses <- cbind(dinosaur_lengths, masses)
...
c.  Calculate the mean mass for each `species` using `dplyr` (no `for` loops).
    ...
    dino_masses %>% 
      group_by(species) %>% 
      summarise(avg_masses = mean(masses))
    ```
...
-   Obtain a list of all of the files with file names matching the pattern `"collar-data-.*.txt"` (using `list.files()`)
    ...
    ```{r}
    download.file(url = "http://www.datacarpentry.org/semester-biology/data/individual_collar_data.zip",
    ...
    list.files(pattern = "collar-data-.*.txt")

    collar_files <- list.files(pattern = "collar-data-.*.txt")

    collar_files
    ```

-   Use a loop to load each of these files into R and make a line plot (using `geom_path()`) for each file with `long` on the `x` axis and `lat` on the `y` axis.

    ```{r}
    num_files <- length(collar_files)
    ...
    for (i in 1:num_files){
      ...
      print(
        ggplot(data = data, mapping = aes(x = long, y = lat)) +
          geom_path() +
          ggtitle(collar_files[i])
      )
    }
    ```
...
b.  Add code to the loop to calculate the minimum and maximum latitude in the file, and store these values, along with the name of the file, in a data frame.
    ...
    for (i in 1:num_files){
      ...
      results$max_lat[i] <- max(data$lat)
...
These last few weeks with iterations and R/Github connects have been a bit difficult for me, but it might just be that time of the semester where things really feel like they're piling up! I've started watching the actual lecture recordings for Thursday morning sessions, and they have been helping tremendously. The final project does not worry me too much, as I already have a lot of data cleaning and organizing completed from far earlier in the semester for one of my own projects. However, the idea of bringing in the concepts from lessons 11-12 do make me a bit nervous when it comes to using them in my project. This is just because, as stated before, it is the content in this class that I am the most unfamiliar/uncomfortable with thus far.
