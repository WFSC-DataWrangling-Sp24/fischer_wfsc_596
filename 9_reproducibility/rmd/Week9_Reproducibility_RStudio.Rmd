...
```{r eval = FALSE}
# Windows
data <- read_csv('/Users/Gracie/Documents/WFSC-596/shrub-dimensions-labeled.csv')
```
...
```{r eval = FALSE}
"Documents/WFSC-596/shrub-dimensions-labeled"
```
...
```{r eval = FALSE}
data <- read_csv('data_raw/shrub-dimesnions-labeled.csv)
```
...
```{r eval = FALSE}
write_csv(data, "data_clean/shrubs-modified/csv")
```
...
```{r eval = FALSE}
data <- read-csv("../data_raw/shrub-dimensions-labeled.csv")
```
