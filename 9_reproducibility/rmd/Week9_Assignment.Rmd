...
a.  Load the 3 dataframes (surveys, species, plots) into R using `read_csv()`. Make sure your paths are *relative*.
    ...
    plots <- read_csv("https://ndownloader.figshare.com/files/3299474")
    species <- read_csv("https://ndownloader.figshare.com/files/3299483")
...
b.  Copy the answers from Week 8 Assignment, Questions 2d-f, into the code chunk below.
    ...
    ```{r}
    species_surveys <- inner_join(surveys, species, join_by(species_id))

    dipodomys_plots <- inner_join(species_surveys, plots, join_by(plot_id))%>% filter(genus == "Dipodomys")%>% select(year, genus, species, weight, plot_type)

    dipodomys_plots

    ggplot(surveys, mapping = aes(x = weight, y = hindfoot_length, color = species_id)) +   geom_point()+
      labs(x = "Weight (g)", y = "Hindfoot length (mm)") + scale_x_log10()

    total_weights <- surveys %>% filter(!is.na(weight))

    ggplot(total_weights, aes(x = weight)) + 
      geom_histogram() +
...
c.  Save the output of the code from 2d as a new dataframe. Then, save that resulting dataframe as a csv file in the clean data sub-directory using the `write_csv()` function.
    ...
    ```{r}
    write_csv(total_weights, file =  "../clean_data/total_weights.csv")
    ```

d.  Save the ggplots from 2e and 2f into the outputs folder using the `ggsave()` function.

    ```{r}
    ggplot(surveys, mapping = aes(x = weight, y = hindfoot_length, color = species_id)) +
    geom_point() +
    labs(x = "Weight (g)", y = "Hindfoot length (mm)") +
    scale_x_log10()

    ggsave(file = "../outputs/weight_hindfoot_scatter.png")
    ```

e.  

    ```{r}
    total_weights <- surveys %>%
    filter(!is.na (weight))
    ggplot(total_weights, aes(x = weight)) +
    geom_histogram() +
    labs(x = "Weight (g)", y = "Number of individuals") +
    ...
    ggsave(file = "../outputs/weight_genus_histos.png")
    ```
...
download.file(url = "https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-pal.219.3&entityid=002f3893385f710df69eeebe893144ff",
              destfile = '/Users/Gracie/Documents/WFSC-596/raw_data/adelie.csv')
...
download.file(url = "https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-pal.220.3&entityid=e03b43c924f226486f2f0ab6709d2381",
              destfile = "/Users/Gracie/Documents/WFSC-596/raw_data/gentoo.csv")
...
download.file(url = "https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-pal.221.2&entityid=fe853aa8f7a59aa84cdd3197619ef462",
              destfile = "/Users/Gracie/Documents/WFSC-596/raw_data/chinstrap.csv")

adelie <- read_csv("C:/Users/Gracie/Documents/WFSC-596/raw_data/adelie.csv")
gentoo <- read_csv("C:/Users/Gracie/Documents/WFSC-596/raw_data/gentoo.csv")
chinstrap <- read_csv("C:/Users/Gracie/Documents/WFSC-596/raw_data/chinstrap.csv")
```
...
b.  Copy and then run the code that you wrote to combine the three above datasets and have the output match the `penguins` dataframe from the `palmerpenguins` dataframe.
    ...
    penguins_gf <- bind_rows(chinstrap, gentoo, adelie) %>% 
      ...
      mutate(species = word(species, start = 1, end = 1, sep = fixed(" "))) %>% 
...
c.  Run the `setdiff()` function to make sure that your code worked (it shouldn't have any issues, but it is good to check!)
    ...
    ```{r}
    setdiff(penguins, penguins_gf)
    ```

d.  Save your version of the cleaned penguins data into your clean data folder.

    ```{r}
    write_csv(penguins_gf, file =  "../clean_data/penguins_gf.csv")
    ```
