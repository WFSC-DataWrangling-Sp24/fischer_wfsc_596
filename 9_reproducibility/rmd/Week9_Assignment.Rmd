...
b.  Copy the answers from Week 8 Assignment, Questions 2d-f, into the code chunk below.
    ...
    \`\`\`{r}d} species_surveys \<- inner_join(surveys, species, join_by(species_id))
    ...
    ggplot(surveys, mapping = aes(x = weight, y = hindfoot_length, color = species_id)) + geom_point() + labs(x = "Weight (g)", y = "Hindfoot length (mm)") + scale_x_log10()

    total_weights \<- surveys %\>% filter(!is.na (weight)) ggplot(total_weights, aes(x = weight)) + geom_histogram() + labs(x = "Weight (g)", y = "Number of individuals") + facet_wrap(\~species_id, scales = "free_y")
