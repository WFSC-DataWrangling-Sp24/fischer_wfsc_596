...
#when x is 4, nothing happens (no results are given)
```
...
```{r}
est_biomass <- function(veg_type, volume){
  if(veg_type == "shrub"){
    mass <- 2.65 * volume^0.9
  } else if (veg_type == "grass"){
    mass <- 0.65 * volume^1.2
...
```{r}
est_biomass("shrub", 1.6)
est_biomass("grass", 1.6)
est_biomass("tree", 1.6)
```
...
#regardless that it already checked the first statement, it will check the second and run through again
```
...
#will only check second condition if first condition was false
```
...
est_biomass("shrub", 1.6, 4)
est_biomass("shrub", 1.6, 10)
est_biomass("grass", 1.6, NA)
```
...
```{r}
x <- 7
#if x is more than 5, multiple by 4. if not, multiply by 10
